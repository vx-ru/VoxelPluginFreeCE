...
bool FVoxelData::Undo(TArray<FVoxelIntBox>& OutBoundsToUpdate)
{
	...
	UndoRedo.CurrentFrameUniqueId = UndoRedo.UndoUniqueIds.Pop(EAllowShrinking::No);
...
bool FVoxelData::Redo(TArray<FVoxelIntBox>& OutBoundsToUpdate)
{
	...
	UndoRedo.CurrentFrameUniqueId = UndoRedo.RedoUniqueIds.Pop(EAllowShrinking::No);
	...
	if (ensure(UndoRedo.LeavesWithRedoStackStack.Num() > 0)) UndoRedo.LeavesWithRedoStackStack.Pop(EAllowShrinking::No);
